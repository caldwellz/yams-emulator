...
void m68k_set_fc(unsigned int new_fc)
{
    ...
    if (new_fc != fc) {
        LogDebug("Changed function code to 0x%01X (%s); previously kept for %i accesses\n", new_fc, functionCodes[new_fc], fc_count);
        fc = new_fc;
        fc_count = 0;
    }
    else
...
unsigned int m68k_read_memory_8(unsigned int address)
{
    if (address <= YAMS_GetMaxMemAddress())
        ...
        LogWarn("Read byte outside memory at $%06X\n", address);
    }
...
unsigned int m68k_read_memory_16(unsigned int address)
{
    if (address <= YAMS_GetMaxMemAddress())
        ...
        // TODO: read from peripheral space
        LogWarn("Read word outside memory at $%06X\n", address);
    }
...
unsigned int m68k_read_memory_32(unsigned int address)
{
    if (address <= YAMS_GetMaxMemAddress())
        ...
        // TODO: read from peripheral space
        LogWarn("Read long outside memory at $%06X\n", address);
    }
...
void m68k_write_memory_8(unsigned int address, unsigned int value)
{
    if (address == DBG_ADDRESS)
        LogDebug("Wrote       0x%02X to debug address\n", value);
    else {
        if (address <= YAMS_GetMaxMemAddress())
            YAMS_WriteMemory8(address, value);
        else {
            // TODO: write to peripheral space
            LogWarn("Ignored write of       0x%02X outside memory at $%06X\n", value, address);
        }
    }
}
...
{
    if (address == DBG_ADDRESS)
        LogDebug("Wrote     0x%04X to debug address\n", value);
    else {
        if (address <= YAMS_GetMaxMemAddress())
            YAMS_WriteMemory16(address, value);
        else {
            // TODO: write to peripheral space
            LogWarn("Ignored write of     0x%04X outside memory at $%06X\n", value, address);
        }
    }
}
...
{
    if (address == DBG_ADDRESS)
        LogDebug("Wrote 0x%08X to debug address\n", value);
    else {
        if (address <= YAMS_GetMaxMemAddress())
            YAMS_WriteMemory32(address, value);
        else {
            // TODO: write to peripheral space
            LogWarn("Ignored write of 0x%08X outside memory at $%06X\n", value, address);
        }
    }
}
